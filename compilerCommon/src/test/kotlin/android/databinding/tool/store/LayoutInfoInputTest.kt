...
class LayoutInfoInputTest {
  ...
  @Test
  fun empty() {
    val input = create(added = emptyList(), removed = emptyList(), incremental = false, log = null)
  ...
  fun increment_add() {
    ...
    val input = create(added = listOf(added), removed = emptyList(), incremental = true, log = log)

    val prevMapping = mapOf("foo" to fooClass, "bar" to barClass)
    assertThat(input.invalidatedClasses, `is`(emptySet()))
  ...
  fun increment_delete() {
    ...
    log.classInfoLog.addMapping("bar", barClass)
    bar.delete()
    val input = create(added = emptyList(), removed = listOf(bar), incremental = true, log = log)
    val prevMapping = mapOf("foo" to fooClass)
  ...
  fun increment_addConfigLayout() {
    ...
    val input = create(added = listOf(fooLand), removed = emptyList(), incremental = true, log = log)

    val prevMapping = mapOf("bar" to barClass)
  ...
  fun increment_removeConfigLayout() {
    ...
    log.classInfoLog.addMapping("bar", barClass)
    fooLand.delete()
    val input = create(added = emptyList(), removed = listOf(fooLand), incremental = true, log = log)
  ...
  fun increment_dependency() {
    ...
    // update baz
    FileUtils.touch(baz)
    val input = create(added = listOf(baz), removed = emptyList(), incremental = true, log = log)
    val prevMapping = mapOf("bar" to barClass)
  ...
  fun increment_external_dependency_newDependency() {
    ...
    val bazClass =
      GenClassInfoLog.GenClass(
        qName = "com.Baz",
        variables = mapOf("var1" to "Int", "var2" to "String"),
        modulePackage = "y.x",
        implementations = setOf(GenClassInfoLog.GenClassImpl(tag = "foo", merge = false, qualifiedName = "com.Baz.Impl")),
      )
    libClassInfoLog.addMapping("baz", bazClass)
    val libInfo = File(dependencyClassFolder, "com.baz." + DataBindingBuilder.BINDING_CLASS_LIST_SUFFIX)
    ...
    val input = create(added = emptyList(), removed = emptyList(), incremental = true, log = log)

    val prevMapping = mapOf("bar" to barClass)

    assertThat(input.invalidatedClasses, `is`(setOf("com.Foo")))
    assertThat(input.existingBindingClasses.mappings(), `is`(mapOf("bar" to barClass, "baz" to bazClass)))
    assertThat(input.filesToConsider, `is`(setOf(foo)))
    assertThat(input.unchangedLog.classInfoLog.mappings(), `is`(prevMapping))
  }
  ...
  fun increment_external_dependency_unrelatedChange() {
    ...
    val prevMapping = mapOf("bar" to barClass, "foo" to fooClass)

    assertThat(input.invalidatedClasses, `is`(emptySet()))
    assertThat(input.existingBindingClasses.mappings(), `is`(mapOf("bar" to barClass, "foo" to fooClass, "baz" to bazClass)))
    assertThat(input.filesToConsider, `is`(emptySet()))
    assertThat(input.unchangedLog.classInfoLog.mappings(), `is`(prevMapping))
  }
  ...
  fun deterministic_log_output() { // Regression test for bug 187340555
    ...
    // Write the log file
    val input = create(added = emptyList(), removed = emptyList(), incremental = true, log = null, packageName = "com.example")
    input.saveLog(log)

    // Read the log file that was just written
    val logFile = File(artifactFolder, "com.example${DataBindingBuilder.BINDING_CLASS_LIST_SUFFIX}")
  ...
  private fun createClass(qName: String, variables: Map<String, String> = emptyMap()): GenClassInfoLog.GenClass {
    return GenClassInfoLog.GenClass(
      qName = qName,
      variables = variables,
      modulePackage = "android.x",
      implementations = setOf(GenClassInfoLog.GenClassImpl(tag = "foo", merge = false, qualifiedName = "${qName}Impl")),
    )
  }
