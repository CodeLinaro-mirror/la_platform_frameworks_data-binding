...
/**
 * Parameters for BaseDataBinder class. It also includes the logic to decide which files need to be processes, which ones are invalid etc.
 */
class LayoutInfoInput(val args: Args) {
  ...
  companion object {
    @VisibleForTesting const val LOG_FILE_NAME = "base_builder_log.json"
  ...
  private val allInfoFiles by lazy(LazyThreadSafetyMode.NONE) { FileUtil.listAndSortFiles(args.infoFolder, arrayOf("xml"), true) }

  private val groupedInfoFiles: Map<String, List<File>> by
    lazy(LazyThreadSafetyMode.NONE) { allInfoFiles.groupBy { getBareLayoutName(it.name) } }
  ...
  /**
   * Binding classes that are inherited from depenendecies + prev run they were generated in the previous run and are not affected in this
   * run (incremental)
   */
  val existingBindingClasses: GenClassInfoLog by
    lazy(LazyThreadSafetyMode.NONE) {
      if (args.incremental) {
  ...
  /**
   * These are classes that are either deleted, changed, outOfDate or has been potentially affected by another change (e.g. dependent layout
   * file)
   */
  val invalidatedClasses: Set<String> by
    lazy(LazyThreadSafetyMode.NONE) { invalidOutputs.mapNotNull { baseBinderLog.classInfoLog.mappings()[it]?.qName }.toSet() }
  ...
  val filesToConsider by lazy {
    if (args.incremental) {
      ...
      invalidOutputs.flatMap { groupedInfoFiles[it] ?: emptyList() }.toSet()
    } else {
  ...
  val unchangedLog: LayoutInfoLog by
    lazy(LazyThreadSafetyMode.NONE) {
      ...
      baseBinderLog.classInfoLog.mappings().forEach { mapping ->
        if (!invalidOutputs.contains(mapping.key)) {
          ...
          baseBinderLog.getDependencies(mapping.key).forEach { out.addDependency(mapping.key, it) }
        }
      }
      out
    }

  private val invalidOutputs: Set<String> by
    lazy(LazyThreadSafetyMode.NONE) {
  ...
  fun saveLog(myLog: LayoutInfoLog) {
    ...
    merged.serialize(File(args.artifactFolder, "${args.packageName}${DataBindingBuilder.BINDING_CLASS_LIST_SUFFIX}"))
  }

  // separate serializable input args so that we can use gradle's worker unit
  data class Args(
    val outOfDate: List<File>,
