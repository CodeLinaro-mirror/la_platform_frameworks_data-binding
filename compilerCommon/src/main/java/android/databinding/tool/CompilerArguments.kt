...
@Suppress("unused")
/**
 * This class is used to pass information from the build system into the data binding compiler. It can serialize itself to a given list of
 * annotation processor options and read itself from there.
 */
...
constructor(
  ...
  // comma separated list of package names for direct dependencies that are directly accessible in
  // this compilation.
  ...
  init {
    Preconditions.check(!(incremental && !isEnableV2), "Incremental annotation processing is not supported by data binding V1")
    Preconditions.check(
  ...
  /**
   * Creates a copy of the arguments but sets the version to v1 and package to the given package. This is used when we need to run a
   * compatibility compilation for v1 dependencies.
   */
  ...
  fun toMap(): Map<String, String> {
    ...
    apiFile?.let { args[PARAM_API_FILE] = it.path }
  ...
  companion object {
    /** legacy arguments are used by blaze, will be removed once blaze is updated. it cannot happen in lock step. */
    ...
    // accessible
    // it looks like [pkg1, pkg2]. Java does not distinguish between empty string vs null (absent)
    // so we are
    ...
    fun readFromOptions(options: Map<String, String>): CompilerArguments {
      return CompilerArguments(
        ...
        dependencyArtifactsDir = File((options[PARAM_DEPENDENCY_ARTIFACTS_DIR] ?: options[LEGACY_PARAM_DEPENDENCY_ARTIFACTS_DIR])!!),
        layoutInfoDir = File((options[PARAM_LAYOUT_INFO_DIR] ?: options[LEGACY_PARAM_LAYOUT_INFO_DIR])!!),
        ...
        aarOutDir = (options[PARAM_AAR_OUT_DIR] ?: options[LEGACY_PARAM_AAR_OUT_DIR])?.let { File(it) },
        exportClassListOutFile =
          (options[PARAM_EXPORT_CLASS_LIST_OUT_FILE] ?: options[LEGACY_PARAM_EXPORT_CLASS_LIST_OUT_FILE])?.let { File(it) },
        enableDebugLogs = stringToBoolean(options[PARAM_ENABLE_DEBUG_LOGS]),
        printEncodedErrorLogs = stringToBoolean(options[PARAM_PRINT_ENCODED_ERROR_LOGS]),
        ...
        dependenciesRFiles = options[PARAM_DEPENDENCIES_R_FILES]?.let { stringToFileList(it) },
        mergedDependenciesRFile = options[PARAM_MERGED_DEPENDENCIES_R_FILE]?.let { File(it) },
      )
    }
    ...
    fun stringToFileList(fileListValue: String): List<File> {
      return fileListValue.splitToSequence(File.pathSeparatorChar).map { rFile -> File(rFile) }.toList()
    }
