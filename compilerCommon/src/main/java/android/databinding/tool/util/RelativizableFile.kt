...
/**
 * Represents an absolute path that could be split into two components: the path to a base directory, and the relative path from that base
 * directory. The absolute path is always available (non-null), whereas the base directory and the relative path may not be available (they
 * are nullable).
 */
class RelativizableFile private constructor(val baseDir: File?, file: File /* Relative if baseDir != null, absolute if baseDir == null */) {
  ...
  companion object {
    ...
    /** Creates a [RelativizableFile] with a base directory and a relative path from the base directory. */
    @JvmStatic
    ...
    /**
     * Creates a [RelativizableFile] with a base directory and a relative path from the base directory if the base directory is given and it
     * is a parent of the given absolute path. Otherwise, this method creates a [RelativizableFile] with the given absolute path only.
     */
    @JvmStatic
