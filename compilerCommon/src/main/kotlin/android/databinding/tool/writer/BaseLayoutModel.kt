...
class BaseLayoutModel(val variations: List<LayoutFileBundle>, val getRPackage: ((String, String) -> String)?) {
  private val COMPARE_FIELD_NAME =
    Comparator<BindingTargetBundle> { first, second ->
  ...
  val importsByAlias = variations.flatMap { it.imports }.associate { Pair(it.name, it.type) }
   ...
   */
  fun layoutConfigurationMembership(target: BindingTargetBundle): Pair<List<String>, List<String>> {
    return variations
      .partition { it.bindingTargetBundles.any { otherTarget -> otherTarget.id == target.id && otherTarget.isUsed } }
      .mapEach { it.map(LayoutFileBundle::getDirectory).sorted() }
  }

  private fun getScopeNames(scope: JavaScope): MutableMap<Any, String> {
    return scopedNames.getOrPut(scope) { mutableMapOf() }
  }
  ...
  fun fieldName(variable: ResourceBundle.VariableDeclaration): String {
    return getScopeNames(JavaScope.FIELD).getOrPut(variable) { getUniqueName(JavaScope.FIELD, "m${readableName(variable)}") }
  }

  fun setterName(variable: ResourceBundle.VariableDeclaration): String {
    return getScopeNames(JavaScope.SETTER).getOrPut(variable) { "set${readableName(variable)}" }
  }

  fun getterName(variable: ResourceBundle.VariableDeclaration): String {
    return getScopeNames(JavaScope.GETTER).getOrPut(variable) { "get${readableName(variable)}" }
  }
  ...
  fun generateImplInfo(): Set<GenClassInfoLog.GenClassImpl> {
    return variations.map { GenClassInfoLog.GenClassImpl.from(it) }.toSet()
  }
}
