...
class BaseLayoutBinderWriter(val model: BaseLayoutModel, val libTypes: LibTypes) {
  ...
  fun write() = javaFile(binderTypeName.packageName(), createType()) { addFileComment("Generated by data binding compiler. Do not edit!") }

  private fun createType() =
    classSpec(binderTypeName) {
  ...
  private fun createStaticInflaters(): List<MethodSpec> {
    ...
    val viewGroupParam = parameterSpec(ANDROID_VIEW_GROUP, "root") { addAnnotation(nullable) }
    val viewParam = parameterSpec(ANDROID_VIEW, "view") { addAnnotation(nonNull) }
    val componentParam = parameterSpec(dataBindingComponent, "component") { addAnnotation(nullable) }
    val rLayoutFile = CodeBlock.of("$T.$N", ClassName.get(model.modulePackage, "R", "layout"), model.baseFileName)
    ...
    return listOf(
      methodSpec("inflate") {
        ...
        addStatement(
          "return inflate($N, $N, $N, $T.getDefaultComponent())",
          inflaterParam,
          viewGroupParam,
          attachToRootParam,
          dataBindingUtil,
      ...
      methodSpec("inflate") {
        ...
        addStatement(
          "return $T.<$T>inflateInternal($N, $L, $N, $N, $N)",
          viewDataBinding,
          binderTypeName,
          inflaterParam,
          rLayoutFile,
      ...
      methodSpec("inflate") {
        ...
        returns(binderTypeName)
        addAnnotation(nonNull)
        addStatement("return inflate($N, $T.getDefaultComponent())", inflaterParam, dataBindingUtil)
      },
      methodSpec("inflate") {
        addModifiers(Modifier.PUBLIC, Modifier.STATIC)
        addParameter(inflaterParam)
        ...
        addStatement(
          "return $T.<$T>inflateInternal($N, $L, null, false, $N)",
          viewDataBinding,
          binderTypeName,
          inflaterParam,
          rLayoutFile,
      ...
      methodSpec("bind") {
        ...
        returns(binderTypeName)
        addStatement("return bind($N, $T.getDefaultComponent())", viewParam, dataBindingUtil)
      },
      methodSpec("bind") {
        addModifiers(Modifier.PUBLIC, Modifier.STATIC)
        addParameter(viewParam)
        ...
        addStatement("return ($T)bind($N, $N, $L)", binderTypeName, componentParam, viewParam, rLayoutFile)
      },
    )
  }
    ...
    return model.variables.flatMap { variable ->
      ...
      listOf(
        methodSpec(model.setterName(variable)) {
          ...
          val param =
            parameterSpec(typeName, variable.name) {
  ...
  fun generateClassInfo(): GenClassInfoLog.GenClass {
    return GenClassInfoLog.GenClass(
      ...
      variables = model.variables.associate { Pair(it.name, it.type.toTypeName(libTypes, model.importsByAlias).toString()) },
