...
fun ViewBinder.generatedClassInfo() =
...
private class JavaFileGenerator(private val binder: ViewBinder, private val useLegacyAnnotations: Boolean) {
  private val annotationPackage = if (useLegacyAnnotations) "android.support.annotation" else "androidx.annotation"
  ...
  private val fieldNames =
    NameAllocator().apply {
      ...
      binder.bindings.forEach { binding -> newName(binding.name, binding) }
    }
  private val rootFieldName = fieldNames.newName("rootView")

  fun create() =
    javaFile(binder.generatedTypeName.packageName(), typeSpec()) { addFileComment("Generated by view binder compiler. Do not edit!") }

  private fun typeSpec() =
    classSpec(binder.generatedTypeName) {
  ...
  private fun rootViewField() =
    fieldSpec(rootFieldName, binder.rootNode.type) {
      addModifiers(PRIVATE, FINAL)
      addAnnotation(nonNull)
    }

  private fun bindingFields() =
    binder.bindings.map { binding ->
      fieldSpec(binding.name, binding.type) {
        ...
        if (binding.isRequired) {
          ...
          addJavadoc(renderConfigurationJavadoc(binding.presentConfigurations, binding.absentConfigurations))
  ...
  private fun constructor() = constructorSpec {
    ...
    addParameter(parameterSpec(binder.rootNode.type, rootFieldName) { addAnnotation(nonNull) })
    addStatement("this.$rootFieldName = $rootFieldName")

    binder.bindings.forEach { binding ->
      val name = fieldNames.get(binding)
      addParameter(parameterSpec(binding.type, name) { addAnnotation(if (binding.isRequired) nonNull else nullable) })
      addStatement("this.$1N = $1N", name)
    }
  }

  private fun rootViewGetter() =
    methodSpec("getRoot") {
  ...
  private fun oneParamInflate() =
    methodSpec("inflate") {
      ...
      val inflaterParam = parameterSpec(ANDROID_LAYOUT_INFLATER, "inflater") { addAnnotation(nonNull) }
      addParameter(inflaterParam)

      addStatement("return inflate($N, null, false)", inflaterParam)
    }

  private fun threeParamInflate() =
    methodSpec("inflate") {
      // TODO addJavadoc
      ...
      val inflaterParam = parameterSpec(ANDROID_LAYOUT_INFLATER, "inflater") { addAnnotation(nonNull) }
      val parentParam = parameterSpec(ANDROID_VIEW_GROUP, "parent") { addAnnotation(nullable) }
      ...
      addStatement("$T root = $N.inflate($L, $N, false)", ANDROID_VIEW, inflaterParam, binder.layoutReference.asCode(), parentParam)
  ...
  private fun mergeInflate() =
    methodSpec("inflate") {
      ...
      val inflaterParam = parameterSpec(ANDROID_LAYOUT_INFLATER, "inflater") { addAnnotation(nonNull) }
      val parentParam = parameterSpec(ANDROID_VIEW_GROUP, "parent") { addAnnotation(nonNull) }
      ...
      addStatement("$N.inflate($L, $N)", inflaterParam, binder.layoutReference.asCode(), parentParam)
  ...
  private fun bind() =
    methodSpec("bind") {
      ...
      val rootParam = parameterSpec(ANDROID_VIEW, localNames.newName("rootView")) { addAnnotation(nonNull) }
      ...
      binder.bindings.forEach { binding ->
        ...
        val viewInitializer =
          if (binding === rootBinding) {
            ...
            CodeBlock.of("$T.findChildViewById($N, $L)", viewBindings, rootParam, binding.id.asCode())
          }
      ...
      addStatement("return new $T($L)", binder.generatedTypeName, CodeBlock.join(constructorParams, ",$W"))

      if (id != null) {
        endControlFlow()

        val missingId = localNames.newName("missingId")
        addStatement("$T $missingId = $N.getResources().getResourceName($id)", String::class.java, rootParam)

        // String.concat(String) produces less bytecode than '+' (StringBuilder).
        addStatement("throw new $T($S.concat($missingId))", NullPointerException::class.java, "Missing required view with ID: ")
      }
    }
