...
data class BindingMethodsCompat(val methods: List<BindingMethodCompat>) {
  companion object {
    @JvmStatic
    ...
    private fun create(annotation: android.databinding.BindingMethods): BindingMethodsCompat {
      return BindingMethodsCompat(
        annotation.value
          .map {
            try {
              BindingMethodCompat(type = safeType { it.type.java.canonicalName }, attribute = it.attribute, method = it.method)
  ...
  data class BindingMethodCompat(val type: String, val attribute: String, val method: String)
}
