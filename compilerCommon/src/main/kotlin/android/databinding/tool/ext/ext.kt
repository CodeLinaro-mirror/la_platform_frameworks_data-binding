...
/**
 * We keep track of these to be cleaned manually at the end of processing cycle. This is really bad but these codes are from a day where
 * javac would be re-created (hence safely static). Now we need to clean them because javac is not re-created anymore between compilations.
 *
 * Eventually, we should move to a better model similar to the UserProperty stuff in IJ source.
 */
private val mappingHashes = CopyOnWriteArrayList<MutableMap<*, *>>()

fun cleanLazyProps() {
  mappingHashes.forEach { it.clear() }
}
...
data class XmlResourceReference(val namespace: String?, val type: String, val name: String, val creating: Boolean)
...
fun String.toTypeName(libTypes: LibTypes, imports: Map<String, String>): TypeName {
  return this.toTypeName(libTypes = libTypes, imports = imports, useReplacements = true)
}
...
private fun String.toTypeName(libTypes: LibTypes, imports: Map<String, String>?, useReplacements: Boolean): TypeName {
  return this.toTypeName(useAndroidX = libTypes.useAndroidX, imports = imports, useReplacements = useReplacements)
}

private fun String.toTypeName(useAndroidX: Boolean, imports: Map<String, String>?, useReplacements: Boolean): TypeName {
  if (this.endsWith("[]")) {
    val qType =
      this.substring(0, this.length - 2).trim().toTypeName(useAndroidX = useAndroidX, imports = imports, useReplacements = useReplacements)
  ...
  if (genericEnd >= 0) {
    ...
    if (genericStart >= 0) {
      ...
      val typeParamsQualified =
        splitTemplateParameters(typeParams).map {
          it.toTypeName(useAndroidX = useAndroidX, imports = imports, useReplacements = useReplacements)
        }
      val klass =
        this.substring(0, genericStart).trim().toTypeName(useAndroidX = useAndroidX, imports = imports, useReplacements = useReplacements)
      return ParameterizedTypeName.get(klass as ClassName, *typeParamsQualified.toTypedArray())
    }
  }
  if (useReplacements) {
...
private val PRIMITIVE_TYPE_NAME_MAP =
  mapOf(
