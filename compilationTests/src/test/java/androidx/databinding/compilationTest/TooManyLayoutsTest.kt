...
class TooManyLayoutsTest : DataBindingCompilationTestCase() {
  ...
  fun tooManyLayouts() {
    loadApp()
    // 3000 is a random number that does not hit the # of static references limit but still fairly
    // large
    // for any reasonable project
    (0 until 3000).forEach { copyTestData("layout/basic_layout.xml", "app/src/main/res/layout/layout_$it.xml") }
  ...
  fun tooManyModules() {
    ...
    val appDeps = (0 until moduleCount).map { "implementation project(':module$it')" }.joinToString(System.lineSeparator())
    val settings = (0 until moduleCount).map { "include ':module$it'" }.joinToString(System.lineSeparator())
    loadApp(mapOf(KEY_DEPENDENCIES to appDeps, KEY_SETTINGS_INCLUDES to "include ':app'\n$settings"))
    (0 until moduleCount).forEach {
      ...
      (0 until layoutsPerModule).forEach { layoutId ->
        copyTestData("layout/basic_layout.xml", "module$it/src/main/res/layout/module${it}_${layoutId}_layout.xml")
      }
    }
    val result = assembleDebug()
    assertWithMessage(result.error).that(result.resultCode).isEqualTo(0)
  ...
  fun tooManyModules_dependingOnEachother() {
    val moduleCount = 5
    val layoutsPerModule = 25
    val appDeps = (0 until moduleCount).map { "implementation project(':module0')" }.joinToString(System.lineSeparator())
    val settings = (0 until moduleCount).map { "include ':module$it'" }.joinToString(System.lineSeparator())
    loadApp(mapOf(KEY_DEPENDENCIES to appDeps, KEY_SETTINGS_INCLUDES to "include ':app'\n$settings"))
    (0 until moduleCount).forEach {
      val deps =
      ...
      loadModule("module$it", mapOf(KEY_NAMESPACE to "com.example.module$it", KEY_DEPENDENCIES to deps)) // add dependency ?
      (0 until layoutsPerModule).forEach { layoutId ->
        copyTestData("layout/basic_layout.xml", "module$it/src/main/res/layout/module${it}_${layoutId}_layout.xml")
      }
    }
