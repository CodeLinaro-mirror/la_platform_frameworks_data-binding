...
/**
 * Loads intermediates from dependencies which are in V1, generates their code and creates a compiler chef specific to them. (to be able to
 * treat them like a v2 dependency).
 */
class ProcessExpressionsFromV1Compat(
  ...
  /** Returns a CompilerChef if we find V1 dependencies and generate code for them. Null if all dependencies are in v2 */
  fun generate(): CompilerChef? {
    ...
    fun isModuleInV2(modulePackage: String): Boolean {
      return isModuleInV2Lookup.getOrPut(modulePackage) {
        ...
        val typeElement = processingEnvironment.elementUtils.getTypeElement(mapperClass)
        typeElement != null
      }
    }
    // mapping from key (layoutName) to generated code QName (or base class)
    val classMapping = mutableMapOf<String, String>()
    val compatBundle = ResourceBundle(args.modulePackage, ModelAnalyzer.getInstance().libTypes.useAndroidX)
    intermediates
      .flatMap { it.extractBundles() }
      .filterNot { layoutFileBundle -> isModuleInV2(layoutFileBundle.modulePackage) }
      .forEach {
        classMapping[it.fileName] = it.fullBindingClass
        compatBundle.addLayoutBundle(it, false)
      }
    return writeResourceBundle(resourceBundle = compatBundle, compilerArgs = args.copyAsV1(COMPAT_PACKAGE))
  }

  /** Generates the code for v1 compat, returns the newly generated CompilerChef. Returns null if we don't generate anything. */
  private fun writeResourceBundle(resourceBundle: ResourceBundle, compilerArgs: CompilerArguments): CompilerChef? {
    ...
    if (compilerChef.hasAnythingToGenerate()) {
      ...
      if (
        compilerArgs.isApp != compilerArgs.isTestVariant ||
