 ...
 *
 * <p>
 * It is generated from the set of bindable Intermediate files from dependencies and the properties in this module.
 *
 * <p>
 * This class decides the final values that will be assigned to the BR fields. When used with Features (instant apps), it may read existing
 * classes to ensure that the same ID is used for the same fields between dependent features.
 */
class BindableBag(
  ...
  init {
    val modulePackageProps = createPackageProps(pkg = compilerArgs.modulePackage, properties = moduleProperties, captureValues = false)
    val brFiles = loadPreviousBRFiles(generationalClassUtil = Context.generationalClassUtil!!, captureValues = true) + modulePackageProps

    val brPackagesToGenerate =
      if (compilerArgs.isFeature) {
    ...
    val brsWithoutValues = brFiles.flatMap { packageProps -> packageProps.properties.values.filter { it.value == null }.map { it.name } }
  ...
  private fun createPackageProps(pkg: String, properties: Set<String>, captureValues: Boolean): PackageProps {
    val processed =
      if (captureValues) {
        ...
        if (typeElement == null) {
          ...
          properties.map { prop ->
            val value = fields.firstOrNull { it.simpleName.toString() == prop }?.constantValue as? Int // might happen with blaze
            Property(prop, value)
          }
        }
      } else {
  ...
  /** Load BR files which only exist as dependencies on the current feature so we need to generate their BR classes. */
  ...
  /**
   * Read BR classes from dependencies and convert them to PackageProps classes. If captureValues is true, this method will find the actual
   * TypeElement and read the field values. This will allow us to keep those values the same in the current module.
   */
  private fun loadPreviousBRFiles(generationalClassUtil: GenerationalClassUtil, captureValues: Boolean): List<PackageProps> {
    val brFiles = generationalClassUtil.load(GenerationalClassUtil.ExtensionFilter.BR, Intermediate::class.java)
    return brFiles
      .filter { compilerArgs.modulePackage != it.`package` }
      .map { createPackageProps(it.`package`, it.getProperties(), captureValues) }
  }
  ...
  companion object {
    ...
    fun loadFeatureInfo(featureInfoDir: File?): FeaturePackageInfo {
      ...
      if (file == null || !file.exists()) {
        L.e("DataBinding compiler args must include the feature info json when" + " compiling a feature")
  ...
  /** holds the information about a Bindable property, possibly with a value if its BR id is already generated. */
  ...
  /** Keeps the final mapping for a BR class. Props are ordered by key so that re-runs of the same code generates the same output. */
  ...
  class IdBag(private val newIdOffset: Int) {
    ...
    fun buildMapping() = BRMapping(idMapping.map { Pair(it.key, it.value) }.sortedBy { it.first })
  }
}
