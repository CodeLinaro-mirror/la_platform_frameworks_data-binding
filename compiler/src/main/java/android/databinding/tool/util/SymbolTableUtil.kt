...
data class Resources(val symbolTables: ImmutableList<SymbolTable>?) {
  ...
  /**
   * Returns the prefix to be used in R class references in Java/Kotlin. For example: "android." for "android.R.color.white", "com.my.lib."
   * for "com.my.lib.R.string.text" and an empty string for default R class reference e.g. "R.attr.my_attr".
   *
   * This method accepts data-binding specific "types", e.g. using "text" (for "string" resources) or "intArray" ( for "array" resources).
   */
...
data class SymbolTable constructor(val rPackage: String, val resources: ImmutableMultimap<String, String>) {
...
fun parseRTxtFiles(localRFile: File?, dependenciesRFiles: List<File>?, mergedDependenciesRFile: File?): Resources {
...
fun parseLocalRTxt(file: File): SymbolTable {
  file.useLines {
    ...
    if (!iterator.hasNext()) error("Incorrect package-aware R.txt format. " + "Failed to parse file: ${file.absolutePath}")
    iterator.next()
    // Second line is local package we can ignore
    if (!iterator.hasNext()) error("Resource list needs to contain the local package. " + "Failed to parse file: ${file.absolutePath}")
    val localPackage = iterator.next()
    if (localPackage != "local") error("Illegal local package '$localPackage' in file ${file.absolutePath}")
...
fun readResources(lines: Iterator<String>): ImmutableMultimap<String, String> {
  ...
  while (lines.hasNext()) {
    ...
    if (chunks.size < 2 || (chunks[0] != STYLEABLE && chunks.size != 2)) error("Illegal line in R.txt: '$line'")
...
fun addResource(type: String, name: String, resourcesBuilder: ImmutableMultimap.Builder<String, String>) {
  ...
  ResourceExpr.R_OBJECT_TO_RESOURCE_TYPE[type]?.forEach { resourcesBuilder.put(it, name) }
