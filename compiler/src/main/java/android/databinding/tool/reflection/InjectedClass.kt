...
/**
 * A class that can be used by ModelAnalyzer without any backing model. This is used for ViewDataBinding subclasses that haven't been
 * generated yet, but we still want to resolve methods and fields for them.
 *
...
class InjectedClass(private val mClassName: String, private val mSuperClass: String) : ModelClass() {
  ...
  override val isObservable by lazy(LazyThreadSafetyMode.NONE) { superclass.isObservable }

  override val superclass by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().findClass(mSuperClass, null)!! }

  override val jniDescription: String by lazy(LazyThreadSafetyMode.NONE) { TypeUtil.getInstance().getDescription(this) }
  ...
  override val typeName: TypeName by
