...
/** This is the implementation of ModelClass for the annotation processor. It relies on AnnotationAnalyzer. */
class AnnotationClass(@JvmField val typeMirror: TypeMirror) : ModelClass() {
  ...
  override val componentType by lazy(LazyThreadSafetyMode.NONE) { computeComponentType() as ModelClass? }
  ...
  private fun findInterface(interfaceType: TypeMirror): DeclaredType? {
    ...
    if (typeUtil.isSameType(interfaceType, typeUtil.erasure(typeMirror))) {
      ...
      if (foundInterface == null) {
        L.e("Detected " + interfaceType + " type for " + typeMirror + ", but not able to find the implemented interface.")
        return null
      }
    }
    if (foundInterface.kind != TypeKind.DECLARED) {
      L.e("Found " + interfaceType + " type for " + typeMirror + ", but it isn't a declared type: " + foundInterface)
      return null
    }
  ...
  override val isInterface by
    lazy(LazyThreadSafetyMode.NONE) {
      typeMirror.kind == TypeKind.DECLARED && (typeMirror as DeclaredType).asElement().kind == ElementKind.INTERFACE
    }

  override val isGeneric by
    lazy(LazyThreadSafetyMode.NONE) { typeMirror.kind == TypeKind.DECLARED && (typeMirror as DeclaredType).typeArguments.isNotEmpty() }
  ...
  override val minApi by lazy(LazyThreadSafetyMode.NONE) { extractTargetApi() ?: super.minApi }

  override val typeArguments by
    lazy(LazyThreadSafetyMode.NONE) {
      if (typeMirror.kind == TypeKind.DECLARED) {
  ...
  override fun isAssignableFrom(that: ModelClass?): Boolean {
    ...
    if (isIncomplete || other.isIncomplete) {
      ...
      if (myTypeArguments.size == otherTypeArguments.size && myErasure.isAssignableFrom(otherErasure)) {
  ...
  override val allMethods by
    lazy(LazyThreadSafetyMode.NONE) {
      if (typeMirror.kind == TypeKind.DECLARED) {
        ...
        ElementFilter.methodsIn(members).map { AnnotationMethod(declaredType, it) as ModelMethod }
      } else {
        emptyList()
      }
    }

  override val superclass by
    lazy(LazyThreadSafetyMode.NONE) {
      val superClass =
        if (typeMirror.kind == TypeKind.DECLARED) {
  ...
  private val computedCanonicalName by
    lazy(LazyThreadSafetyMode.NONE) {
      // see b/144300600 for the fallback to typeMirror itself
  ...
  private val computedErasure by
    lazy(LazyThreadSafetyMode.NONE) {
  ...
  private val computedJniDescription by lazy(LazyThreadSafetyMode.NONE) { TypeUtil.getInstance().getDescription(this) }

  override val jniDescription: String
    get() = computedJniDescription

  override val allFields by
    lazy(LazyThreadSafetyMode.NONE) {
      if (typeMirror.kind == TypeKind.DECLARED) {
        val declaredType = typeMirror as DeclaredType
        val elementUtils = elementUtils
        val typeElement = declaredType.asElement() as TypeElement
        val members = elementUtils.getAllMembers(typeElement)
        ElementFilter.fieldsIn(members).map { AnnotationField(declaredType, it) as ModelField }
      } else {
        emptyList()
      }
    }

  private val javaCodeRepresentation by lazy(LazyThreadSafetyMode.NONE) { AnnotationTypeUtil.getInstance().toJava(typeMirror) }

  override fun toString() = javaCodeRepresentation

  private val computedTypeName by lazy(LazyThreadSafetyMode.NONE) { ClassName.get(typeMirror) }
