...
abstract class ModelClass {
   ...
   *
   * @return The component type for arrays, the value type for maps, and the element type for lists.
   */
  abstract val componentType: ModelClass?

  /**
   * @return Whether or not this ModelClass can be treated as a List. This means it is a java.util.List, or one of the Sparse*Array classes.
   */
  val isList by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().listTypes.any { it.isAssignableFrom(this) } }

  /** @return whether or not this ModelClass can be considered a Map or not. */
  val isMap by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().mapType.isAssignableFrom(erasure()) }

  /** @return whether or not this ModelClass is a java.lang.String. */
  val isString by lazy(LazyThreadSafetyMode.NONE) { "java.lang.String" == typeName.toString() }
  ...
  /**
   * @return a list of Generic type parameters for the class. For example, if the class is List&lt;T>, then the return value will be a list
   *   containing T. null is returned if this is not a generic type
   */
  abstract val typeArguments: List<ModelClass>?

  /**
   * @return whether this is a type variable. For example, in List&lt;T>, T is a type variable. However, List&lt;String>, String is not a
   *   type variable.
   */
  ...
  val isObject by lazy(LazyThreadSafetyMode.NONE) { "java.lang.Object" == typeName.toString() }
  ...
  val isViewDataBinding by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().viewDataBindingType!!.isAssignableFrom(this) }

  /** @return whether or not this is a ViewBinding subclass. */
  val isViewBinding by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().viewBindingType!!.isAssignableFrom(this) }

  /** @return whether or not this is an Observable type such as ObservableMap, ObservableList, or Observable. */
  // open for injected
  ...
  /** @return whether or not this is an ObservableField, or any of the primitive versions such as ObservableBoolean and ObservableInt */
  val isObservableField by
    lazy(LazyThreadSafetyMode.NONE) {
      val erasure = erasure()
      ModelAnalyzer.getInstance().observableFieldTypes.any { it.isAssignableFrom(erasure) }
    }

  /** @return whether or not this is a LiveData */
  val isLiveData by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().liveDataType?.isAssignableFrom(erasure()) ?: false }

  /** @return whether or not this is a MutableLiveData */
  @Suppress("MemberVisibilityCanBePrivate")
  val isMutableLiveData by
    lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().mutableLiveDataType?.isAssignableFrom(erasure()) ?: false }
  ...
  val isMutableStateFlow by
    lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().mutableStateFlowDataType?.isAssignableFrom(erasure()) ?: false }

  /** @return the name of the simple getter method when this is an ObservableField or LiveData or Flow or `null` for any other type */
  ...
  /**
   * @return the name of the simple setter method when this is an ObservableField or MutableLiveData or MutableStateFlow or `null` for any
   *   other type.
   */
  ...
  /**
   * If this represents a class, the super class that it extends is returned. If this represents an interface, the interface that this
   * extends is returned. `null` is returned if this is not a class or interface, such as an int, or if it is java.lang.Object or an
   * interface that does not extend any other type.
   *
   * @return The class or interface that this ModelClass extends or null.
   */
  abstract val superclass: ModelClass?

  /** @return A String representation of the class or interface that this represents, not including any type arguments. */
  open val canonicalName: String by lazy(LazyThreadSafetyMode.NONE) { erasure().toJavaCode() }

  /** @return The class or interface name of this type or the primitive type if it isn't a reference type. */
  val simpleName: String by lazy(LazyThreadSafetyMode.NONE) { canonicalName.substringAfterLast('.') }
   ...
   *
   * @return The SDK_INT where this method was added. If it is not a framework method, should return 1.
   */
  open val minApi: Int by lazy(LazyThreadSafetyMode.NONE) { SdkUtil.get().getMinApi(this) }
  ...
  val abstractMethods: List<ModelMethod> by lazy(LazyThreadSafetyMode.NONE) { allMethods.filter { it.isAbstract } }
  ...
  val isKotlinUnit by lazy(LazyThreadSafetyMode.NONE) { "kotlin.Unit" == typeName.toString() }

  /**
   * Provides the java code that should be generated for this class when it is used as the declared type of a variable.
   *
   * In data binding, a user declares a type when defining a `&lt;variable&gt;` tag, and they may omit the generic parameter there, e.g.
   * declaring `&lt;variable type="MyGeneric"&gt;` when the actual class is `MyGeneric&lt;T: Foo&gt;`. In that case, we need to keep the
   * type-erased version for public APIs, which gets returned by [toJavaCode]. However, when we declare a variable with the type, e.g. for
   * internal implementations, we need to add the type information, e.g. `MyGeneric&lt;Foo&gt;`, which is returned by this method.
   *
  ...
  val extendsViewStub by lazy(LazyThreadSafetyMode.NONE) { ModelAnalyzer.getInstance().viewStubType!!.isAssignableFrom(this) }

  /**
   * When this is a boxed type, such as Integer, this will return the unboxed value, such as int. If this is not a boxed type, this is
   * returned.
   *
   * @return The unboxed type of the class that this ModelClass represents or this if it isn't a boxed type.
   */
  abstract fun unbox(): ModelClass

  /**
   * When this is a primitive type, such as boolean, this will return the boxed value, such as Boolean. If this is not a primitive type,
   * this is returned.
   *
   * @return The boxed type of the class that this ModelClass represents or this if it isn't a primitive type.
   */
   ...
   * @param staticOnly Whether only static methods should be returned or both instance methods and static methods are valid.
  ...
  private fun getMethods(
    ...
    unwrapObservableFields: Boolean,
  ): List<ModelMethod> {
  ...
  fun getMethods(name: String, numParameters: Int): List<ModelMethod> {
    return allMethods.filter { method ->
      method.isPublic && !method.isStatic && name == method.name && method.parameterTypes.size == numParameters
    }
  }

  /**
   * Returns the public method with the name `name` with the parameters that best match args. `staticOnly` governs whether a static or
   * instance method will be returned. If no matching method was found, null is returned.
   *
   * @param name The method name to find
   * @param args The arguments that the method should accept
   * @param staticOnly true if the returned method must be static or false if it does not matter.
   * @param allowProtected true if the method can be protected as well as public.
   * @param unwrapObservableFields true if the method should check for auto-unwrapping the observable field.
   */
  @JvmOverloads
  fun getMethod(
    ...
    L.d("looking methods for %s. static only ? %s . method count: %d", name, staticOnly, methods.size)
   ...
   *
   * @param name The name of the field or the body of the method name -- can be name(), getName(), or isName().
   * @param staticOnly Whether this should look for static methods and fields or instance versions
  ...
  fun findGetterOrField(name: String, staticOnly: Boolean): Callable? {
    if ("length" == name && isArray) {
      return Callable(Type.FIELD, name, null, ModelAnalyzer.getInstance().loadPrimitive("int"), 0, 0, null, null)
    }
    ...
    for (methodName in methodNames) {
      ...
      for (method in methods) {
        if (method.isPublic && (!staticOnly || method.isStatic) && !method.getReturnType(Arrays.asList(*method.parameterTypes)).isVoid) {
          var flags = DYNAMIC
          if (method.isStatic) {
          ...
          if (method.isBindable) {
            ...
            L.d("backing field for method %s is %s", method.name, if (backingField == null) "NOT FOUND" else backingField.name)
    ...
    return Callable(Callable.Type.FIELD, name, setterFieldName, fieldType, 0, flags, null, publicField.bindableAnnotation)
  }
  ...
  fun findInstanceGetter(name: String): ModelMethod? {
    ...
    for (methodName in methodNames) {
      ...
      for (method in methods) {
        if (method.isPublic && !method.isStatic && !method.getReturnType(Arrays.asList(*method.parameterTypes)).isVoid) {
          return method
        }
      }
    }
  ...
  private fun findSetter(getter: ModelMethod, originalName: String): ModelMethod? {
    ...
    for (name in possibleNames) {
      ...
      for (method in methods) {
        ...
        if (parameterTypes != null && parameterTypes.size == 1 && parameterTypes[0] == param && method.isStatic == getter.isStatic) {
          return method
        }
      }
    }
  ...
  fun findMethods(name: String, staticOnly: Boolean): List<ModelMethod> {
    return allMethods.filter { method -> method.isPublic && method.name == name && (!staticOnly || method.isStatic) }
  }
      ...
      if (fieldName.length > 1) {
        ...
        strip =
          if (fieldIdentifier == '_') {
            true
          } else fieldIdentifier == 'm' && Character.isJavaIdentifierStart(start) && !Character.isLowerCase(start)
        if (strip) {
