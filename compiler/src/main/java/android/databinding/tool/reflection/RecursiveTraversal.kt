...
/**
 * This class helps with recursive resolution code (like resolving a type, printing java etc) and avoids going into an infinite loop if the
 * type is recursive or contains itself in an inner loop.
 *
 * It keeps track of local objects and detect if a recursion happens, delegating to the callback to return whatever is desired.
 *
 * Directly use this one if you know the scope or if you don't (e.g. function being called multiple times), use [RecursiveResolutionStack].
 */
class RecursionTracker<T>(
  ...
  /** Adds the item to the list of tracked items if it is not there already. Returns true if added, false otherwise. */
  ...
  /**
   * Removes the last element from stack and checks it is the expected element to detect buggy code which may not control stack properly.
   */
  @Throws(IllegalStateException::class)
  fun popAndCheck(item: T) {
    val removed = items.pop()
    check(item == removed) { "inconsistent reference stack. received $removed expected $item" }
  }
}
