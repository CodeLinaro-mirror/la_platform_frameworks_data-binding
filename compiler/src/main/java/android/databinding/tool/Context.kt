...
/**
 * Simple class to hold all singletons so that it is relatively easier to clean them. We cannot easily get rid of singletons w/o a bigger
 * change so this is a middle ground where we can start clearing them from a central location.
 *
...
object Context {
  ...
  @JvmStatic
  fun init(processingEnvironment: ProcessingEnvironment, args: CompilerArguments) {
    L.setClient(logger)
    ...
    resources = parseRTxtFiles(args.localR, args.dependenciesRFiles, args.mergedDependenciesRFile)
  }

  private fun discoverAndroidX(processingEnvironment: ProcessingEnvironment): Boolean {
    val hasSupportBinding = processingEnvironment.elementUtils.getTypeElement("android.databinding.Observable") != null
    val hasAndroidXBinding = processingEnvironment.elementUtils.getTypeElement("androidx.databinding.Observable") != null
    if (hasAndroidXBinding && hasSupportBinding) {
      L.e(
        "AndroidX Error: Both old and new data binding packages are available in dependencies. Make sure" +
