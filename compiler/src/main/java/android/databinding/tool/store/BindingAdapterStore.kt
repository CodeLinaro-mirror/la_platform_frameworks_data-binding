 ...
 *
 * Even though this class implements {@link Intermediate}, it is non-serializable on purpose. Any future changes in {@link Intermediate}
 * format should be done in this object, by incrementing `version` number but in a JSON safe way. The {@link #upgrade()} method can use the
 * version field to make any further changes on the result object.
 *
 * <p>
 ...
 *
 * <p>
 * This class holds an internal copy for everything added after it is created to ensure that we don't accumulate all adapters from
 * dependencies. When we want to serialize, SetterStore serializes that inner class instead (to persist only the stuff from current module).
 */
@Suppress("ReplacePutWithAssignment")
internal class BindingAdapterStore private constructor(private val useAndroidX: Boolean) : Intermediate {
  @Suppress("unused") @field:SerializedName("version") private var version = 5
  // Intermediate V1
  ...
  @field:SerializedName("renamedMethods") private val renamedMethods = TreeMap<String, TreeMap<String, MethodDescription>>()
  @field:SerializedName("conversionMethods") private val conversionMethods = TreeMap<String, TreeMap<String, MethodDescription>>()
  @field:SerializedName("untaggableTypes") private val untaggableTypes = TreeMap<String, String>()
  @field:SerializedName("multiValueAdapters") private val multiValueAdapters = TreeMap<MultiValueAdapterKey, MethodDescription>()
  // Intermediate V2
  @field:SerializedName("inverseAdapters") private val inverseAdapters = TreeMap<String, TreeMap<AccessorKey, InverseDescription>>()
  @field:SerializedName("inverseMethods") private val inverseMethods = TreeMap<String, TreeMap<String, InverseDescription>>()
  // Intermediate V3
  @field:SerializedName("twoWayMethods") private val twoWayMethods = TreeMap<InverseMethodDescription, String>()

  /** Adapter store that keeps only stuff in this module. Created after this class is sealed. */
  @field:Transient private var currentModuleStore: BindingAdapterStore? = null

  constructor(stores: MutableList<Intermediate>, previousStores: List<BindingAdapterStore>, useAndroidX: Boolean) : this(useAndroidX) {
    previousStores.forEach { merge(it.upgrade() as BindingAdapterStore) }
    stores.forEach { merge(it.upgrade() as BindingAdapterStore) }
  }
  ...
  private fun <K1, K2, V : MethodDescription> Map<K1, Map<K2, V>>.filterOutAndroidSupport(): Map<K1, Map<K2, V>> {
    if (!useAndroidX) {
      return this
    }
    return mapValues { it.value.filterOutAndroidSupportFromMap() }
  }

  private fun <K, V : MethodDescription> Map<K, V>.filterOutAndroidSupportFromMap(): Map<K, V> {
    if (!useAndroidX) {
      return this
    }
    return filterValues { !it.type.androidSupportArtifact() }
  }

  private fun <K : InverseMethodDescription, V> Map<K, V>.filterOutAndroidSupportFromMapByKeys(): Map<K, V> {
    if (!useAndroidX) {
      return this
    }
    return filterKeys { !it.type.androidSupportArtifact() }
  }
  ...
  private fun merge(other: BindingAdapterStore) {
    ...
    untaggableTypes.putAll(
      if (useAndroidX) {
        other.untaggableTypes.filterNot { it.key.androidSupportArtifact() || it.value.androidSupportArtifact() }
      } else {
  ...
  /** Returns all event attributes from inverse descriptions (from inverse adapters and inverse methods) */
  fun collectInverseEvents(): MutableSet<String> {
    val result = mutableSetOf<String>()
    inverseAdapters.values.forEach { it.values.forEach { result.add(it.event) } }
    inverseMethods.values.forEach { it.values.forEach { result.add(it.event) } }
    return result
  }

  fun addRenamedMethod(attribute: String, declaringClass: String, desc: MethodDescription) {
    renamedMethods.getOrPut(attribute) { TreeMap() }.put(declaringClass, desc)
    currentModuleStore?.addRenamedMethod(attribute, declaringClass, desc)
    L.d("STORE addmethod desc %s", desc)
  }

  fun addInverseBindingMethod(attribute: String, declaringClass: String, desc: InverseDescription) {
    inverseMethods.getOrPut(attribute) { TreeMap() }.put(declaringClass, desc)
    currentModuleStore?.addInverseBindingMethod(attribute, declaringClass, desc)
    L.d("STORE addInverseMethod desc %s", desc)
  }

  fun addInverseMethod(from: InverseMethodDescription, to: InverseMethodDescription) {
    val storedToName = twoWayMethods[from]
    if (storedToName != null && to.method != storedToName) {
      throw IllegalArgumentException(
        String.format("InverseMethod from %s to %s does not match expected method '%s'", from, to, storedToName)
      )
    }
    val storedFromName = twoWayMethods[to]
    if (storedFromName != null && from.method != storedFromName) {
      throw IllegalArgumentException(
        String.format("InverseMethod from %s to %s does not match expected method '%s'", to, from, storedFromName)
      )
    }
  ...
  fun addBindingAdapter(attribute: String, key: AccessorKey, desc: MethodDescription) {
    adapterMethods
      .getOrPut(attribute) { TreeMap() }
      .also {
        it[key]?.let { existing ->
          if (existing != desc) {
            L.w("Binding adapter %s already exists for %s! Overriding %s" + " with %s", key, attribute, existing, desc)
          }
        }
      }
      .put(key, desc)
  ...
  fun addInverseBindingAdapter(attribute: String, key: AccessorKey, desc: InverseDescription) {
    inverseAdapters
      .getOrPut(attribute) { TreeMap() }
      .also {
        it[key]?.let { existing ->
          if (existing != desc) {
            L.w("Inverse adapter %s already exists for %s! Overriding %s" + " with %s", key, attribute, existing, desc)
          }
        }
      }
      .put(key, desc)
  ...
  fun addMultiValueAdapter(key: MultiValueAdapterKey, methodDescription: MethodDescription) {
    multiValueAdapters.put(key, methodDescription)
    currentModuleStore?.addMultiValueAdapter(key, methodDescription)
  }

  fun addUntaggableType(types: Array<String>, declaredType: String) {
    types.forEach { untaggableTypes.put(it, declaredType) }
    currentModuleStore?.addUntaggableType(types, declaredType)
  }

  fun addConversionMethod(fromType: String, toType: String, methodDescription: MethodDescription) {
    conversionMethods.getOrPut(fromType) { TreeMap() }.put(toType, methodDescription)
  ...
  /** Runs the given filter on all multiValueAdapters and collects all non-null results from the filter. */
  fun findMultiValueAdapters(filter: (MultiValueAdapterKey, MethodDescription) -> MultiAttributeSetter?): ArrayList<MultiAttributeSetter> {
    val matched = arrayListOf<MultiAttributeSetter>()
    multiValueAdapters.forEach { filter(it.key, it.value)?.let { matched.add(it) } }
    return matched
  }
  ...
  fun findRenamed(attribute: String, filter: (String) -> Boolean): MutableList<String> {
    val maps = renamedMethods[attribute]
    return maps?.let { maps.entries.asSequence().filter { filter(it.key) }.map { it.value.method }.toMutableList() } ?: arrayListOf()
  }

  /** Runs the given function on all inverse methods that has the given attribute. */
  fun forEachInverseMethod(attribute: String, func: (String, InverseDescription) -> Unit?) {
    inverseMethods[attribute]?.forEach { func(it.key, it.value) }
  }

  /**
   * Runs the given function on all conversion methods that has the given attribute. If the function returns a non-null value, returns that
   * value.
   */
  fun findFirstConversionMethod(func: (String, Map<String, MethodDescription>) -> MethodDescription?): MethodDescription? {
  ...
  fun forEachAdapterMethod(attribute: String, func: (AccessorKey, MethodDescription) -> Unit?) {
    adapterMethods[attribute]?.forEach { func(it.key, it.value) }
  }

  /** Runs the given function on all inverse adapters that has the given attribute. */
  fun forEachInverseAdapterMethod(attribute: String, func: (AccessorKey, InverseDescription) -> Unit?) {
    inverseAdapters[attribute]?.forEach { func(it.key, it.value) }
  }
  ...
  companion object {
    ...
    private inline fun <reified K, reified V, reified D> merge(first: TreeMap<K, TreeMap<V, D>>, second: Map<K, Map<V, D>>) {
      second.forEach { key, values ->
        values.let { secondItems ->
          val firstVals = first.getOrPut(key) { TreeMap() }
          secondItems.forEach { v, d -> firstVals.putIfAbsent(v, d) }
        }
      }
    }
  }
