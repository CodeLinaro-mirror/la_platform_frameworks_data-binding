...
class BindingMapperWriterV2(
  ...
  companion object {
    ...
    private val ILLEGAL_ARG_EXCEPTION = ClassName.get("java.lang", "IllegalArgumentException")
    private val STRING = ClassName.get("java.lang", "String")
    private val INTEGER = ClassName.get("java.lang", "Integer")
    ...
    private val SPARSE_INT_ARRAY = ClassName.get("android.util", "SparseIntArray")
    private val SPARSE_ARRAY = ClassName.get("android.util", "SparseArray")
    private val HASH_MAP = ClassName.get(HashMap::class.java)

    @JvmStatic fun createMapperQName(modulePackage: String) = "$modulePackage.$IMPL_CLASS_NAME"

    /**
     * The method that converts a local layout id into a view binding might get fairly large due to the switch statement. This constant
     * controls how many layouts are handled in each function. The main entry point basically finds which function can respond to a given id
     * and calls it directly.
     */
    private const val GET_VIEW_BINDING_CHUNK_SIZE = 50

    /**
     * Java has a limit on the # of lines in a code block (function). This constant controls how many statements we add into a code block.
     * [addChunkedStaticBlock] method uses it to divide long static statement blocks into smaller methods if necessary.
     */
  ...
  private val testOverride: ClassName = ClassName.get(libTypes.bindingPackage, MergedBindingMapperWriter.TEST_CLASS_NAME)
  ...
  init {
    ...
    /**
     * Layout ids might be non-final while generating the mapper for a library. For that case, we generate an internal lookup table that
     * converts an R file into a local known field value.
     */
    val localizedLayoutIdMap = mutableMapOf<String, LocalizedMapping>()

    fun getLocalizedLayoutId(layoutName: String, info: GenClassInfoLog.GenClass): LocalizedMapping {
      ...
      return localizedLayoutIdMap.getOrPut(layoutName) {
        ...
        LocalizedMapping(layoutName = layoutName, localId = id, localIdField = spec, genClass = info)
      }
    }
    allMappings =
      genClassInfoLog.mappings().entries.sortedBy { it.key }.map { getLocalizedLayoutId(it.key, it.value) }.sortedBy { it.localId }
    chunkedMappings = allMappings.chunked(GET_VIEW_BINDING_CHUNK_SIZE)
  }

  val qualifiedName = "$pkg.$className"
  private val dependencyModulePackages = modulePackages.filter { it != pkg }

  private fun getRClass(pkg: String): ClassName {
    return rClassMap.getOrPut(pkg) { ClassName.get(pkg, "R") }
  }

  /**
   * Representation of a layout to generated class mapping that also has a localId which is only valid within this class. Because R files
   * are not final during compilation, we use a class local id to identify these and map from R to this id in initialization.
   */
  data class LocalizedMapping(val localId: Int, val layoutName: String, val localIdField: FieldSpec, val genClass: GenClassInfoLog.GenClass)
  ...
  private fun createLocalizedLayoutIds(builder: TypeSpec.Builder) {
    /**
     * generated code looks like: private static final SparseIntArray INTERNAL_LAYOUT_ID_LOOKUP = new SparseIntArray(99); static {
     * INTERNAL_LAYOUT_ID_LOOKUP.put( foo.bar.R.layout.generic_view, LAYOUT_GENERICVIEW); ... //for all layouts }
     */
    builder.apply {
      // create fields
      allMappings.forEach { addField(it.localIdField) }
      // now create conversion hash map
      // reverse map from ids to values
      val lookupType = SPARSE_INT_ARRAY
      val lookupField =
        FieldSpec.builder(lookupType, LAYOUT_ID_LOOKUP_MAP_NAME)
          .addModifiers(Modifier.PRIVATE, Modifier.FINAL, Modifier.STATIC)
          .initializer("new $T($L)", lookupType, allMappings.size)
          .build()
      addField(lookupField)
      addChunkedStaticBlock(methodPrefix = "internalPopulateLayoutIdLookup", items = allMappings) {
        addStatement("$N.put($L.layout.$L, $N)", lookupField, getRClass(it.genClass.modulePackage), it.layoutName, it.localIdField)
      }
    }
  }

  private fun generateInnerBrLookup(brValueLookup: BindableBag.BRMapping) =
    TypeSpec.classBuilder("InnerBrLookup")
      .apply {
        /**
         * generated code looks like: static final SparseArray<String> sKeys = new SparseArray<String>(214); static {
         * sKeys.put(foo.bar.BR._all, "_all"); ....//for all BRs
         */
        addModifiers(Modifier.PRIVATE, Modifier.STATIC)
        val keysTypeName = ParameterizedTypeName.get(SPARSE_ARRAY, STRING)
        val keysField =
          FieldSpec.builder(keysTypeName, "sKeys")
            .apply {
            ...
            .build()
        addField(keysField)
        addChunkedStaticBlock(methodPrefix = "internalPopulateBRLookup", items = brValueLookup.props) {
          addStatement("$N.put($L, $S)", keysField, it.second, it.first)
        }
      }
      .build()

  private fun generateConvertBrIdToString() =
    MethodSpec.methodBuilder("convertBrIdToString")
      .apply {
        addModifiers(Modifier.PUBLIC)
  ...
  private fun generateInnerLayoutIdLookup() =
    TypeSpec.classBuilder("InnerLayoutIdLookup")
      .apply {
        /**
         * generated code looks like: static final HashMap<String, Integer> sKeys = new HashMap<String, Integer>(214); static {
         * sKeys.put("layout/main_0", foo.bar.R.layout); ....//for all data binding layouts
         */
        addModifiers(Modifier.PRIVATE, Modifier.STATIC)
        val keysTypeName = ParameterizedTypeName.get(HASH_MAP, STRING, INTEGER)
        val keysField =
          FieldSpec.builder(keysTypeName, "sKeys")
            .apply {
              addModifiers(Modifier.STATIC, Modifier.FINAL)
              initializer("new $T($L)", keysTypeName, allMappings.sumBy { it.genClass.implementations.size })
            }
            .build()
        addField(keysField)
        val items =
  ...
  private fun generateGetLayoutId() =
    MethodSpec.methodBuilder("getLayoutId")
      .apply {
        ...
        beginControlFlow("if ($N == null)", tagParam).apply { addStatement("return 0") }.endControlFlow()
        val tmpResult = "tmpVal"
  ...
  /**
   * generates a switch case for the given mappings. looks like: switch(internalId) { case LAYOUT_foo: { if
   * ("layout/layout_foo_0".equals(tag)) { return new Layout58BindingImpl(component, view); } throw new IllegalArgumentException("The tag
   * for layout_58 is invalid. Received: " + tag); } case LAYOUT_bar: { ... } ... }
   *
   * This allows us to create partial (smaller) methods if the app has too many layouts.
   */
  private fun appendSwitchGetForViewDataBinder(
    ...
    methodSpec.apply {
      beginControlFlow("switch($L)", internalIdField)
        .apply {
          mappings.forEach { localizedLayout ->
            ...
            beginControlFlow("case  $N:", layoutIdField)
              .apply {
                ...
                info.implementations.forEach {
                  beginControlFlow("if ($S.equals($L))", "${it.tag}_0", tagField)
                    .apply {
                      val binderTypeName = ClassName.bestGuess(it.qualifiedName)
                      if (it.merge) {
                        addStatement("return new $T($N, new $T[]{$N})", binderTypeName, componentParam, VIEW, viewParam)
                      } else {
                        addStatement("return new $T($N, $N)", binderTypeName, componentParam, viewParam)
                      }
                    }
                    .endControlFlow()
                }
                addStatement("throw new $T($S + $L)", ILLEGAL_ARG_EXCEPTION, "The tag for $layoutName is invalid. Received: ", tagField)
              }
              .endControlFlow()
          }
        }
  ...
  private fun generateGetViewDataBinder(): List<MethodSpec> {
    ...
    val chunks =
      if (!createSubMethods) emptyList()
      else
        chunkedMappings.mapIndexed { index, mappings ->
          MethodSpec.methodBuilder("internalGetViewDataBinding$index")
            .apply {
              ...
              val internalIdParam = ParameterSpec.builder(TypeName.INT, "internalId").build()
    ...
    val impl =
      MethodSpec.methodBuilder("getDataBinder")
        .apply {
          addModifiers(Modifier.PUBLIC)
          addAnnotation(Override::class.java)
          returns(viewDataBinding)
          ...
          val tagField = "tag"
          addStatement("$T $L = $L.get($N)", TypeName.INT, localizedLayoutId, LAYOUT_ID_LOOKUP_MAP_NAME, layoutIdParam)
          // output looks like:
          // localize layout id from R.layout.XY to local private constant
          // switch(layoutId)
          //    case known_layout_id
          //             verify, generate impl and return
          beginControlFlow("if($L > 0)", localizedLayoutId)
            .apply {
              addStatement("final $T $L = $N.getTag()", OBJECT, tagField, viewParam)
              beginControlFlow("if($L == null)", tagField)
                .apply { addStatement("throw new $T($S)", RUNTIME_EXCEPTION, "view must have a tag") }
                .endControlFlow()
              if (createSubMethods) {
                // only call the chunk that might have it
                val methodIndexField = "methodIndex"
                addStatement("// find which method will have it. -1 is necessary because" + " first id starts with 1")
                addStatement("$T $L = ($N - 1) / $L", TypeName.INT, methodIndexField, localizedLayoutId, GET_VIEW_BINDING_CHUNK_SIZE)
                beginControlFlow("switch($N)", methodIndexField).apply {
                  chunks.forEachIndexed { index, methodSpec ->
                    beginControlFlow("case $L:", index).apply {
                      addStatement("return $N($N, $N, $L, $L)", methodSpec, componentParam, viewParam, localizedLayoutId, tagField)
                    }
                    endControlFlow()
                  }
                }
  ...
  private fun generateGetViewArrayDataBinder() =
    MethodSpec.methodBuilder("getDataBinder")
      .apply {
        ...
        addParameter(viewParam)
        addParameter(layoutIdParam)
        beginControlFlow("if($N == null || $N.length == 0)", viewParam, viewParam).apply { addStatement("return null") }.endControlFlow()
        ...
        beginControlFlow("if($L > 0)", localizedLayoutId)
          .apply {
            ...
            beginControlFlow("if(tag == null)")
              .apply { addStatement("throw new $T($S)", RUNTIME_EXCEPTION, "view must have a tag") }
              .endControlFlow()
            beginControlFlow("switch($N)", localizedLayoutId)
              .apply {
                allMappings.forEach { localizedMapping ->
                  val mergeImpls = localizedMapping.genClass.implementations.filter { it.merge }
                  if (mergeImpls.isNotEmpty()) {
                    ...
                    beginControlFlow("case $N:", layoutIdField)
                      .apply {
                        mergeImpls.forEach {
                          beginControlFlow("if($S.equals(tag))", "${it.tag}_0")
                            .apply {
                              val binderTypeName = ClassName.bestGuess(it.qualifiedName)
                              addStatement("return new $T($N, $N)", binderTypeName, componentParam, viewParam)
                            }
                        ...
                        addStatement("throw new $T($S + tag)", ILLEGAL_ARG_EXCEPTION, "The tag for $layoutName is invalid. Received: ")
                      }
                      .endControlFlow()
                  }
                }
  ...
  private fun generateCollectDependencies(): MethodSpec {
    return MethodSpec.methodBuilder("collectDependencies")
      .apply {
        addAnnotation(Override::class.java)
        addModifiers(Modifier.PUBLIC)
        val listType = ParameterizedTypeName.get(ClassName.get(ArrayList::class.java), dataBinderMapper)
        returns(ParameterizedTypeName.get(ClassName.get(List::class.java), dataBinderMapper))
        addStatement("$T result = new $T($L)", listType, listType, dependencyModulePackages.size)
  ...
  /**
   * Helper function to generate static code blocks that avoid java method limit. Based on the given chunk size, this either generates a
   * code section like:
   *
   * static { // handle item 1 // handle item 2 ... }
   *
   * or if the list if bigger than [chunkSize], it generates something like:
   *
   * private static void prefix1() { // handle item 1 // handle item 2 ... }
   *
   * private static void prefix2() { // handle item 1000 // handle item 1001 ... }
   *
   * static { prefix1() prefix2() ... }
   */
  private fun <T> TypeSpec.Builder.addChunkedStaticBlock(
    ...
    if (items.size <= chunkSize) {
      ...
      addStaticBlock(CodeBlock.builder().apply { items.forEach { this.addItem(it) } }.build())
    } else {
      // divide it into methods to avoid java method size limit
      val methods =
        items.chunked(chunkSize).mapIndexed { index, chunk ->
          MethodSpec.methodBuilder("$methodPrefix$index")
            .addModifiers(Modifier.STATIC, Modifier.PRIVATE)
            .addCode(CodeBlock.builder().apply { chunk.forEach { this.addItem(it) } }.build())
            .build()
        }
      methods.forEach { this.addMethod(it) }
      // add 1 static block to call all
      addStaticBlock(CodeBlock.builder().apply { methods.forEach { addStatement("$N()", it) } }.build())
    }
  }
}
