...
class BindingMapperWriter(
  ...
  fun write(brValueLookup: BindableBag.BRMapping) =
    kcode("") {
        nl("package $pkg;")
        ...
        block("class $className $extends") {
          ...
          nl("@Override")
          block(
            "public ${libTypes.viewDataBinding} getDataBinder(${libTypes.dataBindingComponent} bindingComponent, android.view.View view, int layoutId)"
          ) {
            layoutBinders
            ...
            if (generateTestOverride) {
              block("if(mTestOverride != null)") { nl("return mTestOverride.getDataBinder(bindingComponent, view, layoutId);") }
            }
            nl("return null;")
          }
          nl("@Override")
          block(
            "public ${libTypes.viewDataBinding} getDataBinder(${libTypes.dataBindingComponent} bindingComponent, android.view.View[] views, int layoutId)"
          ) {
            layoutBinders
              .filter { it.isMerge }
              .groupBy { it.layoutname }
            ...
            if (generateTestOverride) {
              block("if(mTestOverride != null)") { nl("return mTestOverride.getDataBinder(bindingComponent, views, layoutId);") }
            }
            nl("return null;")
          }
          nl("@Override")
          block("public int getLayoutId(String tag)") {
            block("if (tag == null)") { nl("return 0;") }
            ...
            if (generateTestOverride) {
              block("if(mTestOverride != null)") { nl("return mTestOverride.getLayoutId(tag);") }
            }
            nl("return 0;")
          }
          nl("@Override")
          block("public String convertBrIdToString(int id)") {
            ...
            if (generateTestOverride) {
              block("if(value == null && mTestOverride != null)") { nl("return mTestOverride.convertBrIdToString(id);") }
            }
            nl("return value;")
          }
          ...
          block("private static class InnerBrLookup") {
            ...
            block("static") {
              ...
              brValueLookup.props.forEach { tab("sKeys.put(${it.second}, \"${it.first}\");") }
            }
          }
        }
      }
