...
class ExprModelExt {
  ...
  fun getUniqueName(base: String, scope: Scope, isPublic: kotlin.Boolean): String {
    ...
    var i = 0
    while (usedFieldNames[scope]!!.contains(candidate) || (checkFields && usedFieldNames[Scope.FIELD]!!.contains(candidate))) {
      i++
...
fun BindingTarget.superConversion(variable: String): String {
  return if (resolvedType != null && resolvedType.extendsViewStub) {
    ...
    "($variable != null) ? " + "${resolvedType.toJavaCode()}.bind((android.view.View) $variable) " + ": null"
  } else {
...
val BindingTarget.constructorParamName by lazyProp { target: BindingTarget -> target.model.getConstructorParamName(target.readableName) }
...
val Expr.fieldName by lazyProp { expr: Expr -> expr.model.getUniqueFieldName("m${expr.readableName.capitalizeUS()}", false) }
...
val Expr.listenerClassName by lazyProp { expr: Expr -> expr.model.getUniqueFieldName("${expr.resolvedType.simpleName}Impl", false) }

val Expr.oldValueName by lazyProp { expr: Expr -> expr.model.getUniqueFieldName("mOld${expr.readableName.capitalizeUS()}", false) }

fun Expr.scopedName(): String =
  when (Scope.currentScope) {
...
val Expr.setterName by lazyProp { expr: Expr -> expr.model.getUniqueMethodName("set${expr.readableName.capitalizeUS()}", true) }

val Expr.onChangeName by lazyProp { expr: Expr -> expr.model.getUniqueMethodName("onChange${expr.readableName.capitalizeUS()}", false) }

val Expr.getterName by lazyProp { expr: Expr -> expr.model.getUniqueMethodName("get${expr.readableName.capitalizeUS()}", true) }

fun Expr.isVariable() = this is IdentifierExpr && this.isDynamic

val Expr.dirtyFlagSet by lazyProp { expr: Expr -> FlagSet(expr.invalidFlags, expr.model.flagBucketCount) }

val Expr.invalidateFlagSet by lazyProp { expr: Expr -> FlagSet(expr.id) }

val Expr.shouldReadFlagSet by versionedLazy { expr: Expr -> FlagSet(expr.shouldReadFlags, expr.model.flagBucketCount) }
...
val Expr.conditionalFlags by lazyProp { expr: Expr ->
  arrayListOf(FlagSet(expr.getRequirementFlagIndex(false)), FlagSet(expr.getRequirementFlagIndex(true)))
}

fun Binding.toAssignmentCode(): String {
  val fieldName: String
  if (this.target.viewClass.equals(this.target.interfaceType)) {
...
val LayoutBinder.requiredComponent by lazyProp { layoutBinder: LayoutBinder ->
  val requiredFromBindings =
    layoutBinder.bindingTargets.flatMap { it.bindings }.firstOrNull { it.bindingAdapterInstanceClass != null }?.bindingAdapterInstanceClass
  val requiredFromInverse =
    layoutBinder.bindingTargets
      .flatMap { it.inverseBindings }
      .firstOrNull { it.bindingAdapterInstanceClass != null }
      ?.bindingAdapterInstanceClass
...
fun FlagSet.localValue(bucketIndex: Int) = if (localName == null) binaryCode(bucketIndex) else "$localName${getWordSuffix(bucketIndex)}"
...
class LayoutBinderWriter(val layoutBinder: LayoutBinder, val libTypes: LibTypes) {
  ...
  val includedBinders by lazy { layoutBinder.bindingTargets.filter { it.isDataBindingLayout } }

  val variables by lazy { model.exprMap.values.filterIsInstance(IdentifierExpr::class.java).filter { it.isVariable() } }

  val callbacks by lazy { model.exprMap.values.filterIsInstance(LambdaExpr::class.java) }
  ...
  fun calculateIndices() {
    val taggedViews = layoutBinder.bindingTargets.filter { it.isUsed && it.tag != null && !it.isDataBindingLayout }
    taggedViews.filter { it.includedLayout == null }.forEach { indices.put(it, indexFromTag(it.tag)) }
    // put any included layouts after the normal views
    taggedViews.filter { it.includedLayout != null }.forEach { indices.put(it, maxIndex() + 1) }
    val indexStart = maxIndex() + 1
    layoutBinder.bindingTargets
      .filter { it.isUsed && !taggedViews.contains(it) }
      .withIndex()
      .forEach { indices.put(it.value, it.index + indexStart) }
  }

  fun declareIncludeViews() =
    kcode("") {
      nl("@Nullable")
      ...
      nl("static {") {
        ...
        if (!hasBinders) {
          ...
          layoutBinder.bindingTargets
            .filter { it.isUsed && it.isDataBindingLayout }
            .forEach {
              val includeTag = it.tag
              val parent =
                layoutBinder.bindingTargets.firstOrNull { it.isUsed && !it.isBinder && includeTag.equals(it.tag) }
        ...
        if (viewsWithIds.isEmpty()) {
          ...
          viewsWithIds.forEach { tab("sViewsWithIds.put(${it.androidId}, ${indices[it]});") }
        }
      }
      nl("}")
    }
  ...
  fun declareConstructor(minSdk: kotlin.Int) =
    kcode("") {
      ...
      if (hasBaseBinder) {
        ...
        nl("private $className(${libTypes.dataBindingComponent} bindingComponent, $parameterType root, Object[] bindings) {") {
          tab("super(bindingComponent, $superParam, ${model.observables.size}") {
            layoutBinder.sortedTargets.filter { it.id != null }.forEach { tab(", ${fieldConversion(it)}") }
            tab(");")
          }
        }
      } else {
      ...
      taggedViews.forEach {
        ...
        if (!it.isBinder) {
          ...
          if (it.supportsTag() && it.tag != null && (rootTagsSupported || it.tag.startsWith("binding_"))) {
            ...
            if (originalTag != null && !originalTag.startsWith("@{")) {
              ...
              if (originalTag.startsWith("@")) {
                ...
                if (originalTag.startsWith("@android:")) {
                  ...
                  val rPackage = Context.resources.getRPackagePrefix(null, "string", resourceId)
          ...
          } else if (it.tag != null && !it.tag.startsWith("binding_") && it.originalTag != null) {
  ...
  fun declareCallbackInstances() =
    kcode("// listeners") {
      callbacks
        .groupBy { it.callbackWrapper.minApi }
        .forEach {
          if (it.key > 1) {
            block("if(getBuildSdkInt() < ${it.key})") { it.value.forEach { lambda -> nl("${lambda.fieldName} = null;") } }
            block("else") { it.value.forEach { lambda -> nl("${lambda.fieldName} = ${lambda.generateConstructor()};") } }
          } else {
            it.value.forEach { lambda -> nl("${lambda.fieldName} = ${lambda.generateConstructor()};") }
          }
        }
    }

  fun declareCallbackImplementations() =
    kcode("// callback impls") {
      callbacks
        .groupBy { it.callbackWrapper }
        .forEach {
          val wrapper = it.key
          ...
          if (shouldReturn) {
            lambdas.forEach { it.callbackExprModel.ext.forceLocalize.add(it.expr) }
          }
          // b: 123260053
          fun checkCanReturn(lambda: LambdaExpr) {
            if (shouldReturn) {
          ...
          block("public final ${wrapper.method.returnType.canonicalName} ${wrapper.listenerMethodName}(${wrapper.allArgsWithTypes()})") {
            ...
            if (lambdas.size == 1) {
              ...
              block("switch(${CallbackWrapper.SOURCE_ID})") {
                ...
                if (shouldReturn) {
                  block("default:") { nl("return ${wrapper.method.returnType.defaultValue()};") }
                }
              }
            }
          }
  ...
  fun declareInvalidateAll() =
    kcode("") {
      nl("@Override")
      block("public void invalidateAll()") {
        val fs = FlagSet(layoutBinder.model.invalidateAnyBitSet, layoutBinder.model.flagBucketCount)
        block("synchronized(this)") {
          for (i in (0..(mDirtyFlags.buckets.size - 1))) {
            tab("${mDirtyFlags.localValue(i)} = ${fs.localValue(i)};")
          }
        }
        includedBinders.filter { it.isUsed }.forEach { binder -> nl("${binder.fieldName}.invalidateAll();") }
        nl("requestRebind();")
      }
    }

  fun declareHasPendingBindings() =
    kcode("") {
      nl("@Override")
      nl("public boolean hasPendingBindings() {") {
        if (mDirtyFlags.buckets.size > 0) {
          tab("synchronized(this) {") {
            val flagCheck = 0.rangeTo(mDirtyFlags.buckets.size - 1).map { "${mDirtyFlags.localValue(it)} != 0" }.joinToString(" || ")
            tab("if ($flagCheck) {") { tab("return true;") }
            tab("}")
          }
          tab("}")
        }
        includedBinders
          .filter { it.isUsed }
          .forEach { binder ->
            tab("if (${binder.fieldName}.hasPendingBindings()) {") { tab("return true;") }
            tab("}")
          }
        tab("return false;")
      }
    ...
    kcode("") {
      ...
      block("public boolean setVariable(int variableId, @Nullable Object variable) ") {
        ...
        if (variables.isNotEmpty()) {
          block("else") { nl("variableSet = false;") }
        }
        tab("return variableSet;")
      }
    }

  fun variableSettersAndGetters() =
    kcode("") {
      variables.forEach {
        if (it.userDefinedType != null) {
          ...
          block("public void ${it.setterName}(${if (it.resolvedType.isPrimitive) "" else "@Nullable "}$argType ${it.readableName})") {
            ...
            if (used) {
              ...
              block("synchronized(this)") {
                mDirtyFlags.mapOr(flagSet) { suffix, index -> nl("${mDirtyFlags.localName}$suffix |= ${flagSet.localValue(index)};") }
              }
          ...
          if (!hasBaseBinder) {
            ...
            block("public $argType ${it.getterName}()") { nl("return ${it.fieldName};") }
          }
        }
      }
    }

  fun declareSetLifecycleOwnerOverride() =
    kcode("") {
      ...
      if (includes.isNotEmpty()) {
        ...
        block("public void setLifecycleOwner(@Nullable ${libTypes.lifecycleOwner} lifecycleOwner)") {
          ...
          includes.forEach { binder -> nl("${binder.fieldName}.setLifecycleOwner(lifecycleOwner);") }
        }
      }
    }
      ...
      nl("protected boolean onFieldChange(int localFieldId, Object object, int fieldId) {") {
        tab("switch (localFieldId) {") {
          model.observables.forEach {
            tab("case ${it.id} :") { tab("return ${it.onChangeName}((${it.resolvedType.toJavaCode()}) object, fieldId);") }
          }
        }
        tab("}")
        tab("return false;")
        ...
        block("private boolean ${it.onChangeName}(${it.resolvedType.toJavaCode()} ${it.readableName}, int fieldId)") {
          block("if (fieldId == ${"".br()})") {
            ...
            if (it is FieldAccessExpr && it.resolvedType.observableGetterName != null) {
              flagSet = it.bindableDependents.map { expr -> expr.invalidateFlagSet }.foldRight(it.invalidateFlagSet) { l, r -> l.or(r) }
            } else {
              flagSet = it.invalidateFlagSet
            }

            block("synchronized(this)") {
              mDirtyFlags.mapOr(flagSet) { suffix, index -> tab("${mDirtyFlags.localName}$suffix |= ${flagSet.localValue(index)};") }
            }
            nl("return true;")
          }
            ...
            .forEach {
              ...
              block("else if (fieldId == ${it.key})") {
                block("synchronized(this)") {
                  ...
                  mDirtyFlags.mapOr(flagSet) { _, index -> tab("${mDirtyFlags.localValue(index)} |= ${flagSet.localValue(index)};") }
                }
                nl("return true;")
              }
            }
  ...
  fun declareViews() =
    kcode("// views") {
      layoutBinder.sortedTargets
        .filter { it.isUsed && (!hasBaseBinder || it.id == null) }
        .forEach {
          val access =
            if (!hasBaseBinder && it.id != null) {
              "public"
  ...
  fun declareListeners() =
    kcode("// listeners") {
      model.exprMap.values
        .filter { it is ListenerExpr }
        .groupBy { it }
        .forEach {
  ...
  fun declareInverseBindingImpls() =
    kcode("// Inverse Binding Event Handlers") {
      layoutBinder.sortedTargets
        .filter { it.isUsed }
        .forEach { target ->
            ...
            block("private $invClass ${inverseBinding.fieldName} = new $invClass($param)") {
                ...
                block("public void onChange()") {
                  if (inverseBinding.inverseExpr != null) {
                    ...
                    nl(
                      "${valueExpr.resolvedType.toJavaCode()} ${valueExpr.name} = ${getterCall.toJava("mBindingComponent", target.fieldName)};"
                    )
                    ...
                    block("synchronized($className.this)") {
                      val flagSet = inverseBinding.chainedExpressions.fold(FlagSet(), { initial, expr -> initial.or(FlagSet(expr.id)) })
                      mDirtyFlags.mapOr(flagSet) { _, index -> tab("${mDirtyFlags.localValue(index)} |= ${flagSet.binaryCode(index)};") }
                    }
                    nl("requestRebind();")
                  }
                }
  ...
  fun executePendingBindings() =
    kcode("") {
      ...
      block("protected void executeBindings()") {
        ...
        model.pendingExpressions
          .filter { it.needsLocalField }
          .forEach {
            nl(
              "${it.resolvedType.toDeclarationCode()} ${it.executePendingLocalName} = ${if (it.isVariable()) it.fieldName else it.defaultValue};"
            )
          }
        ...
        if (batch.isNotEmpty()) {
          L.e(
            "could not generate code for %s. This might be caused by circular dependencies." + "Please report on b.android.com. %d %s %s",
            layoutBinder.layoutname,
            batch.size,
            batch[0],
            batch[0].toCode().generate(),
          )
        }
        //
        layoutBinder.sortedTargets
          ...
          .groupBy {
            tmpDirtyFlags
              .mapOr(it.expr.dirtyFlagSet) { _, index ->
          ...
          .forEach {
            block("if (${it.key})") {
              it.value
                .groupBy { Math.max(1, it.minApi) }
                .forEach {
                  val setterValues = kcode("") { it.value.forEach { binding -> nl(binding.toAssignmentCode()).app(";") } }
                  nl("// api target ${it.key}")
                  if (it.key > 1) {
                    block("if(getBuildSdkInt() >= ${it.key})") { nl(setterValues) }
                  } else {
                    nl(setterValues)
                  }
                }
          ...
          .forEach {
            block("if (${it.key})") {
              it.value
                .groupBy { it.expr }
                .map { it.value.first() }
                .forEach { it.componentExpressions.forEach { expr -> nl("this.${expr.oldValueName} = ${expr.toCode().generate()};") } }
            }
          }
        includedBinders.filter { it.isUsed }.forEach { binder -> nl("executeBindingsOn(${binder.fieldName});") }
        layoutBinder.sortedTargets
          .filter { it.isUsed && it.resolvedType != null && it.resolvedType.extendsViewStub }
          .forEach { block("if (${it.fieldName}.getBinding() != null)") { nl("executeBindingsOn(${it.fieldName}.getBinding());") } }
      }
    }

  fun readWithDependants(
    expressionList: List<Expr>,
    justRead: MutableList<Expr>,
    batch: MutableList<Expr>,
    tmpDirtyFlags: FlagSet,
    inheritedFlags: FlagSet? = null,
  ): KCode =
    kcode("") {
      expressionList
        .groupBy { it.shouldReadFlagSet }
        .forEach {
          val flagSet = it.key
          val needsIfWrapper = inheritedFlags == null || !flagSet.bitsEqual(inheritedFlags)
          val expressions = it.value
          val ifClause =
            "if (${tmpDirtyFlags.mapOr(flagSet) { _, index ->
              ...
              expressions
                ...
                .forEach {
                  ...
                  if (!assignedValues.isEmpty()) {
                    ...
                    if (condition != null) {
                      tab("if ($condition) {") { app("", assignment) }
                      tab("}")
                    } else {
                      app("", assignment)
                    }
                    it.value
                      ...
                      .forEach { expr: Expr -> tab(expr.getUpdateRegistrationCall(expr.id, expr.executePendingLocalName)) }
                  }
                  ...
                  it.value.forEach { expr: Expr ->
                    ...
                    expr.dependants
                      .filter { !it.isConditional && it.dependant is TernaryExpr && (it.dependant as TernaryExpr).pred == expr }
                      .map { it.dependant }
                      .groupBy {
                        // group by when those ternaries will be evaluated (e.g. don't set
                        // conditional flags for no reason)
                        ...
                        if (!isBehindTernary) {
                          ...
                          // TODO if it is behind a ternary, we should set it when its
                          // predicate is elevated
                          // Normally, this would mean that there is another code path to
                          // re-read our current expression.
                          // Unfortunately, this may not be true due to the coverage
                          // detection in `expr#markAsReadIfDone`, this may never happen.
                          // for v1.0, we'll go with always setting it and suffering an
                          // unnecessary calculation for this edge case.
                          // we can solve this by listening to elevation events from the
                          // model.
                          ""
                        }
                      }
                      .forEach {
                        ...
                        val cond: (KCode.() -> Unit) = {
                          it.apply {
                            val predicate =
                              if (expr.resolvedType.isNullable) {
  ...
  fun condition(expr: Expr): String? {
    if (expr.canBeEvaluatedToAVariable() && !expr.isVariable()) {
      ...
      val nullables = expr.dependencies.filter { it.isMandatory && it.other.resolvedType.isNullable }.map { it.other }
  ...
  fun declareListenerImpls() =
    kcode("// Listener Stub Implementations") {
      model.exprMap.values
        .filter { it.isUsed && it is ListenerExpr }
        .groupBy { it }
        .forEach {
          val expr = it.key as ListenerExpr
          ...
          nl("public static class ${expr.listenerClassName} $extendsImplements $listenerType{") {
            ...
            tab("@Override")
            tab(
              "public $returnType ${listenerMethod.name}(${
              ...
              val args = parameterTypes.withIndex().joinToString(", ") { "arg${it.index}" }
  ...
  fun declareFactories() =
    kcode("") {
      nl("@NonNull")
      block(
        "public static $baseClassName inflate(@NonNull android.view.LayoutInflater inflater, @Nullable android.view.ViewGroup root, boolean attachToRoot)"
      ) {
        nl("return inflate(inflater, root, attachToRoot, ${libTypes.dataBindingUtil}.getDefaultComponent());")
      }
      nl("@NonNull")
      block(
        "public static $baseClassName inflate(@NonNull android.view.LayoutInflater inflater, @Nullable android.view.ViewGroup root, boolean attachToRoot, @Nullable ${libTypes.dataBindingComponent} bindingComponent)"
      ) {
        nl(
          "return ${libTypes.dataBindingUtil}.<$baseClassName>inflate(inflater, ${layoutBinder.modulePackage}.R.layout.${layoutBinder.layoutname}, root, attachToRoot, bindingComponent);"
        )
      }
      if (!layoutBinder.isMerge) {
        nl("@NonNull")
        ...
        nl("@NonNull")
        block(
          "public static $baseClassName inflate(@NonNull android.view.LayoutInflater inflater, @Nullable ${libTypes.dataBindingComponent} bindingComponent)"
        ) {
          nl(
            "return bind(inflater.inflate(${layoutBinder.modulePackage}.R.layout.${layoutBinder.layoutname}, null, false), bindingComponent);"
          )
        }
        nl("@NonNull")
        ...
        nl("@NonNull")
        block(
          "public static $baseClassName bind(@NonNull android.view.View view, @Nullable ${libTypes.dataBindingComponent} bindingComponent)"
        ) {
  ...
  /**
   * When called for a library compilation, we do not generate real implementations. This code is only kept for backward compatibility
   * reasons as we move to v2. If you change anything here, make sure it is also changed in BaseLayoutBinderWriter.
   */
  ...
  public fun writeBaseClass(forLibrary: Boolean, variations: List<LayoutBinder>): String =
    kcode("package ${layoutBinder.`package`};") {
        ...
        layoutBinder.sortedTargets
          ...
          .forEach {
            if (
              variations.count { lb -> lb.sortedTargets.any { bt -> bt.isUsed && bt.id == it.id && bt.includedLayout == null } } ==
                variations.size
            ) {
              tab("@NonNull")
            } else {
        ...
        nl("")
        tab("// variables") { variables.forEach { nl("protected ${it.resolvedType.toJavaCode()} ${it.fieldName};") } }

        tab(
          "protected $baseClassName(@Nullable ${libTypes.dataBindingComponent} bindingComponent, @Nullable android.view.View root_, int localFieldCount"
        ) {
          layoutBinder.sortedTargets.filter { it.id != null }.forEach { tab(", ${it.interfaceClass} ${it.constructorParamName}") }
        }
        tab(") {") {
        ...
        tab("//getters and abstract setters") {
          variables.forEach {
            ...
            nl(
              "public abstract void ${it.setterName}(${if (it.resolvedType.isPrimitive) "" else "@Nullable "}$typeCode ${it.readableName});"
            )
            nl("")
            if (!it.resolvedType.isPrimitive) {
              nl("@Nullable")
            }
            block("public $typeCode ${it.getterName}()") { nl("return ${it.fieldName};") }
            nl("")
          }
        }
        tab("@NonNull")
        ...
        tab(
          ...
          if (forLibrary) {
            ...
            tab(
              "return DataBindingUtil.<$baseClassName>inflate(inflater, ${layoutBinder.modulePackage}.R.layout.${layoutBinder.layoutname}, root, attachToRoot, bindingComponent);"
            )
          }
        }
        tab("}")
        ...
        tab(
          ...
          if (forLibrary) {
            ...
            tab(
              "return DataBindingUtil.<$baseClassName>inflate(inflater, ${layoutBinder.modulePackage}.R.layout.${layoutBinder.layoutname}, null, false, bindingComponent);"
            )
          }
        }
        tab("}")
...
/**
 * Runs the given block in the error scope of this scope provider such that any exception thrown will be scoped to that expression for error
 * reporting.
 */
private inline fun android.databinding.tool.processing.scopes.ScopeProvider.inErrorScope(crossinline block: () -> Unit) {
  try {
