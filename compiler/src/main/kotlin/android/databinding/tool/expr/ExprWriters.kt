...
fun CallbackExprModel.localizeGlobalVariables(vararg ignore: Expr): KCode =
  kcode("// localize variables for thread safety") {
    // puts all variables in this model to local values.
    mExprMap.values
      .filter { it.shouldLocalizeInCallbacks() && !ignore.contains(it) }
      .forEach {
        nl("// ${it.toString()}")
...
fun ExecutionPath.toCode(): KCode =
  kcode("") {
    val myExpr = expr
    ...
    if (myTrue != null) {
      ...
      block("if ($condition)") { nl(myTrue.path.toCode()) }
      if (myFalse != null) {
        block("else") { nl(myFalse.path.toCode()) }
      }
    }
  }
