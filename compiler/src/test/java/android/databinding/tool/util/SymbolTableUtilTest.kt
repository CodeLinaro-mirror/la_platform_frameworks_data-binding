...
class SymbolTableUtilTest {
  ...
  fun testParsingMultipleFiles() {
    ...
    val result = parseRTxtFiles(localFile.toFile(), ImmutableList.of(dependencyFile.toFile()), null)
  ...
  fun testParsingWithEmptyDependencies() {
    ...
    val result = parseRTxtFiles(localFile.toFile(), null, null)

    assertEquals(result.symbolTables!!.size, 1)
    assertEquals(result.symbolTables!![0].rPackage, "")

    assertEquals(result.getRPackagePrefix(null, "string", "first"), "")
    val failure = assertFailsWith<Exception> { result.getRPackagePrefix(null, "string", "second") }
