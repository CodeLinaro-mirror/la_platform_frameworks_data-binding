...
class ClassFinderCacheTest {
  ...
  fun simple_fail() {
    ...
    assertThat(loggingFinder.calls, `is`(listOf(DoFindCall("foo", null))))
  ...
  fun import_different() {
    ...
    assertThat(loggingFinder.calls, `is`(listOf(DoFindCall("foo", null), DoFindCall("foo", ImportBag.EMPTY.toImmutable()))))
  ...
  fun import_mutated() {
    val cache = ClassFinderCache(loggingFinder::doFind)
    val fake = loggingFinder.addClassFor("foo")
    val imports = MutableImportBag().also { it.put("baz", "foo.baz") }
    val imports1 = imports.toImmutable()
    val result = cache.find("foo", imports)
    assertThat(loggingFinder.calls, `is`(listOf(DoFindCall("foo", imports1))))
    assertThat(result, `is`(fake))

    imports.put("bazar", "foo.bazar")
    val imports2 = imports.toImmutable()
    val result2 = cache.find("foo", imports2)
    assertThat(loggingFinder.calls, `is`(listOf(DoFindCall("foo", imports1), DoFindCall("foo", imports2))))
    assertThat(result2, `is`(fake))

    val result3 = cache.find("foo", MutableImportBag().also { it.put("baz", "foo.baz") })
  ...
  class LoggingFinder {
    ...
    fun doFind(className: String, imports: ImportBag?): ModelClass? {
      calls.add(DoFindCall(className = className, imports = imports?.toImmutable()))
