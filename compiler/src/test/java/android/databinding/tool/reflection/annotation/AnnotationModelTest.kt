...
class AnnotationModelTest {
  ...
  fun parseGeneric_accessingGenericField() {
    ...
    runProcessorTest(tmpFolder, code) { _, _ ->
      val ownerModel = ModelAnalyzer.getInstance().findClass("com.example.Owner", ImportBag.EMPTY) as AnnotationClass
      val valueField = ownerModel.allFields.first { it.name == "value" }
  ...
  private fun runTest(
    ...
    runProcessorTest(tmpFolder, owner) { _, _ ->
      val ownerModel = ModelAnalyzer.getInstance().findClass(ownerXMLDeclaration, ImportBag.EMPTY) as AnnotationClass
      val methodModel =
        ownerModel.getMethod(name = "get", args = emptyList(), staticOnly = false, allowProtected = false, unwrapObservableFields = true)
          as AnnotationMethod
      assertThat(ownerModel.toDeclarationCode()).isEqualTo(expectedClassDeclaration)
      assertThat(methodModel.returnType.toDeclarationCode()).isEqualTo(expectedMethodReturnTypeDeclaration)
    }
  }
