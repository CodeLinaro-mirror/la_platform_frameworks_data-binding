...
object AndroidDataBinding {
  ...
  fun main(args: Array<String>) {
    /**
     * Sample run: java -jar android-data-binding-fat.jar \ -classInfoOut test-run/info.zip \ -layoutInfoFiles
     * exec/src/test-data/generateBaseClasses/layout-info-1.zip\ -sourceOut test-run/src.zip \ -package ygt.me
     */
    if (args.isEmpty()) {
      printUsageAndExit()
    }
  ...
  private fun printUsageAndExit() {
    ...
    println(
      StringBuilder()
        .apply {
          appendln("Usage: This binary can be used to either process xml resources or generate " + "base classes for data binding.")
  ...
  @Deprecated(
    "use processXML",
    ReplaceWith("processResources(processXmlOptions)", "android.databinding.AndroidDataBinding.processResources"),
  )
  @JvmStatic
  ...
  fun processResources(processXmlOptions: ProcessXmlOptions) {
    ...
    val input = LayoutXmlProcessor.ResourceInput(false, processXmlOptions.resInput, processXmlOptions.resOutput)
    L.setDebugLog(true)
    processor.processResources(input, false, true)
    if (processXmlOptions.shouldZipLayoutInfo()) {
      val outZip = File(processXmlOptions.layoutInfoOutput, "layout-info.zip")
  ...
  /**
   * Creates data binding base classes and ClassInfo for the given parameters. This should be run after data binding exports the info files
   * from layouts, and before javac runs because it generates sources for javac.
   */
  @JvmStatic
  fun generateBaseClasses(options: GenerateBaseClassesOptions) {
    ...
    val sourceFileWriter =
      if (options.zipSourceOutput) {
        ZipFileWriter(options.sourceFileOut)
      } else {
        options.sourceFileOut.mkdirs()
        DataBindingBuilder.GradleFileWriter(options.sourceFileOut.absolutePath)
      }
  ...
  private fun prepareInput(inputs: List<File>): File {
    ...
    inputs.forEach {
      if (it.isFile) {
        ...
        throw IllegalArgumentException("${it.absolutePath} should've been a file a zip file." + "It is not. Does it exist? ${it.exists()}")
      }
    }
    return outFolder
  }
  ...
  private fun createXmlProcessor(processXmlOptions: ProcessXmlOptions): LayoutXmlProcessor {
    ...
    return LayoutXmlProcessor(processXmlOptions.appId, fileWriter, MyFileLookup(), processXmlOptions.useAndroidX)
  }
  ...
  internal class ZipFileWriter @Throws(FileNotFoundException::class) constructor(outZipFile: File) : JavaFileWriter() {
