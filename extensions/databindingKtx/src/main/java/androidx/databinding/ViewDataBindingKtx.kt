...
object ViewDataBindingKtx {
  ...
  private val CREATE_STATE_FLOW_LISTENER = CreateWeakListener { viewDataBinding, localFieldId, referenceQueue ->
    StateFlowListener(viewDataBinding, localFieldId, referenceQueue).listener
  }

  @Suppress("unused") // called by generated code
  @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
  @JvmStatic
  fun updateStateFlowRegistration(viewDataBinding: ViewDataBinding, localFieldId: Int, observable: Flow<*>?): Boolean {
  ...
  internal class StateFlowListener(binder: ViewDataBinding?, localFieldId: Int, referenceQueue: ReferenceQueue<ViewDataBinding>) :
    ...
    private val listener = WeakListener<Flow<Any?>>(binder, localFieldId, this, referenceQueue)
      ...
      observerJob =
        owner.lifecycleScope.launch {
          owner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
            flow.collect { listener.binder?.handleFieldChange(listener.mLocalFieldId, listener.target, 0) }
          }
        }
    }
